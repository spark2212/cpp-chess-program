...
void promotePawn(std::vector<Piece> &pieces, int i, int turnNum, int offerTurn, bool wasOffered, bool wasClaimed, bool display);
...
int main(int argc, char **argv)
{
    ...
    std::cout << "Welcome to ChessGame9.cpp! \n" << std::endl;
    std::string finput{""};
    bool display{true};
    bool resume{false};
    ...
    const std::vector<Piece> startPieces = pieces;
    std::string output1;
    static unsigned long int turnNum{1};
    static unsigned long int oldTurnNum{0};
    int turn{WHITE};
    int oppoTurn{BLACK};
    int oldTurn{EMPTY};
    ...
    //std::cout << std::cin.tellg() << " == std::cin.tellg() and " << std::cin.end << " == std::cin.end " << std::endl;
    ...
    //std::cout << "std::cin.peek() == " << std::cin.peek() << " and magicWord == " << magicWord << std::endl;
    ...
    while(magicWord != "skip" && std::chrono::steady_clock::now() - t <= std::chrono::milliseconds(4000)){
       if(std::cin.tellg() != -1)
        ...
        time++;
        if(time == 1000)
            std::cout << secs++ << "seconds passed... " << std::endl;*/
    }
    ...
    if(std::chrono::steady_clock::now() - t > std::chrono::milliseconds(15000))
    {
        std::cout << "In the future, you may skip these instructions at any time by typing and entering 'skip'. Press enter to begin. " << std::flush;
    ...
    while(gameOver == false)
    {
        ...
        while(gameOver == false)
        {
            ...
            if(turnNum > oldTurnNum || (turnNum == oldTurnNum && turn <= oldTurn))
                display = true;
            ...
            if(buffer == "undo")
            {
                std::cout << outputA << "\nPlease enter the turn number you would like to return to: " << std::endl;
                oldTurnNum = 0;
                oldTurn = EMPTY;
                display = false;
                ...
                {    
                    ...
                    while(oldTurnNum > turnNum || oldTurnNum <= 0)
                    {
                        std::cin.clear();
                        std::cin.ignore(10000, '\n');
                        std::cout << outputA << "Invalid input. \nPlease enter the turn number you would like to return to: " << std::endl;
                        std::cin >> oldTurnNum;
                    }

                    std::cout << outputA << "\nPlease enter the player to move ('white' or 'black'): " << std::endl;
                    std::string player;
                    std::cin >> player;
                    ...
                    std::cout << "turnNum == " << turnNum << " and oldTurnNum == " << oldTurnNum << std::endl;
                    pauseRun(10000, true);
                }while(oldTurnNum > turnNum || (oldTurnNum == turnNum && oldTurn <= turn));

                pieces = startPieces;
                getDrawNum = getDraw(pieces, WHITE, NO_PIECE, true);
                finput.clear();
                std::cin.ignore(10000, '\n');
                finput.append(fSkeleton);
                turn = WHITE;
                turnNum = 1;
                std::cin.seekg(gameStart);
                buffer = "";
                continue;
            }
            ...
            {    
                promotePawn(pieces, i, turnNum, offerTurn, wasOffered, wasClaimed, display); //promotes the pawn to the desired piece
