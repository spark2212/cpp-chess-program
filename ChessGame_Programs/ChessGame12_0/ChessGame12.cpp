...
int main(int argc, char **argv)
{
    ...
    std::cout << "Welcome to ChessGame12.cpp! \n" << std::endl;
    ...
    while(gameOver == false)
    {
        ...
        while(gameOver == false) //Start of inner loop. This loop iterates every attempted move.
        {
            ...
            else if(caughtUp == true)
            {
                ...
                if((buffer == "adjourn" || buffer == "pause") && argc == 2)
                {
                    ...
                    std::cout << oldOutput << "Game adjourned. This game can be resumed by relaunching ChessGame12.cpp \nwith the same command-line arguments and entering 'resume'. Goodye!" << std::endl;
        ...
        {
            if(choice != "review")
                std::cout << outputCopy << "\nThreefold repetition rule " << std::flush;
    ...
    if(decision == true && choice != "quit")
    {
        ...
        while(choice != "review" && choice != "quit" && userInput == true)
        {
            std::cin >> choice;
            tolower(choice);
            if(choice == "yes")
        ...
        if(choice == "review")
        {
            ...
            for(int n = 0; n < oldBoards.size(); n++)
            {
                ...
                for(int z = 0; z < 4; z++)
                {
                    ...
                    oldBoardsAuto[z][n].append(" This game will advance automatically by one move every two seconds. ");
            ...
            std::thread th2(cinThread3, std::ref(navigation), std::ref(go), std::ref(skip));
            th2.detach();
            ...
            while(navigation != "quit")
            {
                ...
                default:
                    ...
                    //std::cout << "navigation == " << navigation << std::flush;
                    pauseRun(1500);
                    valid = false;
                    mode += 20;
                    break;
                ...
                {
                    std::cout << oldBoardsRecent.substr(0, oldBoardsRecent.size()-441) << "Please enter the number of ply (half-moves) you wish to skip: \n\n" << std:: endl;

                    int jumpSize{0};
                    std::cin >> jumpSize;

                    if(navigation == "skip+" && jumpSize < oldBoards.size()-n)
                        n += jumpSize;
                    else if(navigation == "skip-" && jumpSize <= n)
                        n -= jumpSize;
                    else
                        mode -= 20;
...
std::string printTurn(int turnNum, int color, int offerTurn, bool manualClaim, bool wasOffered, bool wasClaimed, bool valid, bool validEntry, int next, int argc) 
{
    ...
    else if(wasOffered == true)
    {
        if(offerTurn == color || (wasClaimed == true && checkBackClaim == 0))
        {
            ...
            //else if(checkBackOffer != 0 && valid == false)
                //turnPhrase.append("Invalid entry. Try again.\n");
        }
