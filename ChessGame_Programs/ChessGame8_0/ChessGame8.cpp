...
std::string printTurn(int turnNum, int color, bool manualClaim, bool wasClaimed, int next);
...
void promotePawn(std::vector<Piece> &pieces, int i, int turnNum, bool wasClaimed);
...
int main(int argc, char **argv)
{
    ...
    output1.append(printTurn(turnNum, turn, manualClaim, wasClaimed, 10));
    ...
    while(gameOver == false)
    {
        ...
        while(1)
        {
            ...
            if(moveEntered == false)
            {
                ...
                else if(wasOffered == true && start == "offer" && end == "draw" && offerNum > 0)
                {
                    //std::string output;
                    output.append(printBoard(pieces, turn, manualClaim, finput, 10));
            ...
            else
            {
                ...
                if(argc == 2)
                {
                    ...
                    finput.append(newInput);
                }
            }

            if(pieces[i].getType() == PAWN && (end[1] == '1' || end[1] == '8'))
            {    
                promotePawn(pieces, i, turnNum, wasClaimed); //promotes the pawn to the desired piece
            ...
            outputA.append(printTurn(turnNum, oppoTurn, manualClaim, wasClaimed, next));
            ...
            output.append(printTurn(turnNum, turn, manualClaim, wasClaimed, next));
...
std::string printTurn(int turnNum, int color, bool manualClaim, bool wasClaimed, int next) 
{
    ...
    if(manualClaim == true && wasClaimed == false)
...
void promotePawn(std::vector<Piece> &pieces, int i, int turnNum, bool wasClaimed)
{
    ...
    output.append(printTurn(turnNum, -1*pieces[i].getTeam(), false, wasClaimed, 10)); //prints the board and pieces
    std::cout << output << std::flush;
    output.clear();
