...
void promotePawn(std::vector<Piece> &pieces, int i, int turnNum, int offerTurn, bool wasOffered, bool wasClaimed);
...
int main(int argc, char **argv)
{
    ...
    if(argc == 2)
    {
        ...
        std::cout << "Enter White's name, or enter 'default' (case-sensitive) to begin the game: ";
        std::cin >> wName;
    ...
    while(gameOver == false)
    {
        ...
        while(gameOver == false/* && countLegalMoves(pieces, turn) != 0*/)
        {
            offerNum = 0;
            oldNext = next;
            ...
            buffer = move;
            //std::cin.ignore(10000, ' ');
            //std::cout << "The next input says " << buffer << std::endl;
            ...
            if(buffer == "i" || buffer == "claim" || buffer == "offer" || buffer == "accept")
            {
                moveEntered = false;
                //std::cout << "moveEntered == false " << std::endl;
                start = buffer;
                std::cin >> end;
            ...
            if(moveEntered == false)
            {
                //std::cin.clear();
                //std::cin.ignore(10000, '\n');

                //std::cout << "conditions met! start == " << start << " and end == " << end << std::endl;
                ...
                else if(start == "offer" && end == "draw" && wasOffered == false)
                {
                    ...
                    /*if(next != 10)
                    {
                        output.append(printBoard(pieces, oppoTurn, manualClaim, finput, next));
                        output.append("Your opponent has extended a draw offer. Enter 'accept' to accept or 'wait' to decide later: ");
                        std::cout << output << std::flush;
                        output.clear();

                        std::string response{""};
                        while(response != "accept" && response != "wait")
                        {
                            std::cin >> response;

                            for(int num = 0; num < response.size(); num++)
                                response[num] = tolower(response[num]);
                            ...
                            else if(response == "wait")
                            {
                                gameOver = false;
                                output.append(printBoard(pieces, turn, manualClaim, finput, 10));
                                output.append(printTurn(turnNum, turn, offerTurn, manualClaim, wasOffered, wasClaimed, 10));
                                std::cout << output << std::flush;
                                output.clear();
                                break;
                            }
                            ...
                            {
                                output.append(printBoard(pieces, oppoTurn, manualClaim, finput, next));
                                output.append("\n\nInvalid response. Enter 'accept' or 'wait': \n");
                                std::cout << output << std::flush;
                                response = "";
                                std::cin.clear();
                                std::cin.ignore(10000, '\n');
                ...
                else if(start == "claim" && end == "draw" && manualClaim == true && wasClaimed == false)
                {
                    ...
                    else
                    {
                        ...
                        //output.append("Claim invalid. All draw claims double as draw offers. \n");
            ...
            /*
                else if(wasClaimed == true && start == "claim" && end == "draw")
                {
                    output.append(printBoard(pieces, turn, manualClaim, finput, 10));
                    output.append("You have already attempted to claim a draw on this turn. \n");
                    output.append(printTurn(turnNum, turn, manualClaim, wasClaimed, 10));
                    std::cout << output << std::flush;
                    output.clear();
                    std::cin.clear();
                    std::cin.ignore(10000, '\n');
                ...
                else if(wasOffered == true && start == "offer" && end == "draw")
                {
                    //std::string output;
                    output.append(printBoard(pieces, turn, manualClaim, finput, 10));
                    output.append("A draw offer has already been extended on this turn. \n");
                    output.append(printTurn(turnNum, turn, manualClaim, wasClaimed, 10));
                    std::cout << output << std::flush;
                    output.clear();
                    std::cin.clear();
            ...
            */    
            ...
            {    
                promotePawn(pieces, i, turnNum, offerTurn, wasOffered, wasClaimed); //promotes the pawn to the desired piece
...
std::vector<std::string> convertMove(std::vector<Piece> &pieces, std::string move, int turn) // search here
{
    ...
    //std::vector<std::string> badFile = badRoute;
    //std::vector<std::string> badRank = badRoute;
    badRoute.push_back(badSpot);
    //badFile.push_back("Z0");
    //badRank.push_back("a9");
...
std::string printTurn(int turnNum, int color, int offerTurn, bool manualClaim, bool wasOffered, bool wasClaimed, int next, bool valid) 
{
    ...
    if(color == WHITE)
    {
        ...
        turnPhrase.append("\n(multiple options may be entered at once): \n");
    }
    else if(color == BLACK)
    {
        turnPhrase.append(std::to_string(turnNum));
...
int getDraw(std::vector<Piece> &pieces, int turn, pieceType pMoved)
{
    ...
    for(int j = 0; j < MAX_SQUARES; j++)
    {
        if(pieces[j].getType() == PAWN && pieces[j].getTeam() == turn)
        {
            ...
            /*else if(pieces[j].getCanEP() == true)
            {
                //pieces = oldPieces;
            ...
            else
            {
                pieces = oldPieces;
                newState.append(1, 'o');
            }*/ //This block comment was the change to EP reading on 6-10-2020 at 11:20am     
        }
    }
