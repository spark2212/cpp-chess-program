...
void promotePawn(std::vector<Piece> &pieces, int i, int viewTurn, int turnNum, int &offerTurn, bool &wasOffered, bool &wasClaimed, bool caughtUp, std::vector<std::string> &oldInputs, std::vector<std::string> &newInputs, int &inputsRead, std::string finput, bool needsEntry);
...
int main(int argc, char **argv)
{
    ...
    std::cout << "Welcome to ChessGame14.cpp! \n" << std::endl;
    ...
    //std::vector<std::vector<std::vector<std::string>>> oldBoardsSETS;
    ...
    while(gameMode != QUIT) //When this loop ends, so does the program
    {
            ...
            while(gameOver == false) //Start of inner loop. This loop iterates every attempted move.
            {
                ...
                if(caughtUp == false) 
                {
                    ...
                    else
                    {
                        ...
                        {
                            if(choice == "review" && gameMode == PLAY)
                ...
                {
                    oldOutput = printBoard(pieces, -1*viewTurn, finput, next);
                    ...
                    if(buffer == "change")
                    {
                        ...
                        std::string changeRules = "\nEnter 'w' to view all future moves from White's perspective, 'b' to view all future moves from Black's perspective, \n'a' to view the board from the perspective of the player to move at all times, \nor 'flip' to flip the current perspective for one turn: ";
                        ...
                        while(newView != "w" && newView != "b" && newView != "a" && newView != "flip")
                        {
                            std::cout << tempOutput << changeRules << std::flush;
                    ...
                    else if((buffer == "adjourn" || buffer == "pause") && argc == 2)
                    {
                        ...
                        std::cout << oldOutput << "Game adjourned. This game can be resumed by relaunching ChessGame14.cpp \nwith the same command-line arguments and entering 'resume'. Goodye!" << std::endl;
                        ...
                        {  
                            ...
                            while(confirm[0] != 'y' && confirm[0] != 'n' && confirm != "cancel")
                            {
                                ...
                                if(adjusted == false)         
                                {
                                    ...
                                    std::cout << "\n'flip', 't' (for toggle), or 'c' to cancel.\nReturn to this position? " << std::flush;
                                }
                                else
                                {
                                    std::cout << "Return to this position? \nEnter 'y' to confirm, 'n' to choose a different position, \n";
                ...
                {    
                    ...
                    promotePawn(pieces, i, viewTurn, turnNum, offerTurn, wasOffered, wasClaimed, caughtUp, oldInputs, newInputs, inputsRead, finput, needsEntry); 
        ...
        if(argc == 2 && newGame == true && choice != "quit" && gameMode != REVIEW)
        {
...
std::string printTurn(int turnNum, int color, int offerTurn, bool manualClaim, bool wasOffered, bool wasClaimed, bool valid, bool validEntry, int next, int argc) 
{
    ...
    turnPhrase.append(", or enter 'change' to change the board perspective: ");
...
void promotePawn(std::vector<Piece> &pieces, int i, int viewTurn, int turnNum, int &offerTurn, bool &wasOffered, bool &wasClaimed, bool caughtUp, std::vector<std::string> &oldInputs, std::vector<std::string> &newInputs, int &inputsRead, std::string finput, bool needsEntry)
{
    ...
    outputTemp.append(printBoard(pieces, -1*viewTurn, finput, 10));
    outputTemp.append(printTurn(turnNum, -1*viewTurn, offerTurn, false, wasOffered, wasClaimed, true, true, 10, 1)); //prints the board and pieces
