...
//Converters
std::vector<std::string> convertMove(std::vector<Piece> &pieces, int turn);
std::vector<std::string> convertMove(std::vector<Piece> &pieces, std::string move, int turn);
std::string convertNotation(std::vector<Piece> oldPieces, std::vector<Piece> pieces, pieceType pMoved, std::string move, std::string start, std::string end, int turn, std::string &finput);
...
//Piece manipulators
std::vector<bool> movePiece(std::vector<Piece> &pieces, int turn);
bool movePiece(std::vector<Piece> &pieces, std::vector<std::string> route, int turn);
...
int main(int argc, char **argv)
{
    ...
    while(gameOver == false)
    {
        ...
        while(1)
        {
            ...
            /*if(std::isspace(std::cin.peek()) == true)
            {
                while(std::cin.ignore()
            }*/  
            
            char nextc = std::cin.get();

            if(tolower(nextc) != 'i' && tolower(nextc) != 'c' && tolower(nextc) != 'o')
            {
                std::cin.putback(nextc);
                //valid = movePiece()
                moveEntered = true;
            }
            else if(tolower(std::cin.peek()) != 'f')
            {
                std::cin.putback(nextc);
                moveEntered = true;
            }
        ...
        */
            //if(buffer == "i" || buffer == "claim" || buffer == "offer")
            if(moveEntered == true)
            {
                valid = movePiece(pieces, turn);
            ...
            else if(moveEntered == false)
            {
                std::cin >> start >> end;

                for(int j = 0; j < start.size(); j++)
                    start[j] = tolower(start);
                for(int j = 0; j < end.size(); j++)
                    end[j] = tolower(end[j]);
            ...
            else
            {
                ...
                if(argc == 2)
                {
                    ...
                    if(turn == WHITE)
                    {
                        ...
                        finput.append(". ");
                        finput.append(start); // comment here
                        finput.append(" ");
                        finput.append(end);
                    }
                    else if(turn == BLACK)
            ...
            outputA.append(printTurn(turnNum, oppoTurn, manualClaim, wasOffered, next));
            ...
            if(argc == 2)
            {
                finput.append(convertNotation(oldPieces, pieces, pMoved, move, start, end, oppoTurn));
            }
...
//Converters//////////////////////////
//This function takes input for a move and converts it from standard chess notation into start and end locations, if necessary.
std::vector<std::string> convertMove(std::vector<Piece> &pieces, int turn) // search here
{
    std::vector<std::string> route;
...
std::string convertNotation(std::vector<Piece> oldPieces, std::vector<Piece> pieces, pieceType pMoved, std::string move, std::string start, std::string end, int turn, std::string &finput)
{
    if(pMoved == KING && end[0] - start[0] == 2)
        ...
        return "O-O-O";
    else if(std::count(move.begin(), move.end(), end) != 0 || std::count(move.begin(), move.end(), "O-O") != 0)
        return move;
    ...
    while(movePiece(oldPieces, test[j], pieces[newPiece].getTeam()) == false)
    {
...
{   
    std::vector<Piece> oldPieces = pieces;
    std::vector<std::string> route = convertMove(pieces, turn);
    std::string start = route[0]; 
    std::string end = route[1];
    std::vector<bool> valid;
...
//This function takes input for a move in chess notation, then performs the entered move, if possible, and returns a bool based on the legality of the attempted move. This is a compound function that calls several other functions to determine the validity of the move before performing it.
bool movePiece(std::vector<Piece> &pieces, std::vector<std::string> route, int turn) 
{   
    std::vector<Piece> oldPieces = pieces;
    std::string start = route[0];
...
//This function requests input for a move, then performs the entered move, if possible, and returns a bool based on the legality of the attempted move. This is a compound function that calls several other functions to determine the validity of the move before performing it.
bool movePiece(std::vector<Piece> &pieces, std::string start, std::string end, int turn) 
{   
    std::vector<Piece> oldPieces = pieces;
    //std::vector<std::string> route = convertMove(pieces, turn);
