    ...
    if(choice == "resume")
    {
        ...
        while(oldLog.good() == true)
        {
            ...
            for(int i = 0; i < newOldInput.size(); i++)
                oldInputs.push_back(newOldInput[i]);
...
std::string convertNotation(std::vector<Piece> oldPieces, std::vector<Piece> pieces, pieceType pMoved, std::string move, std::string start, std::string end, int turn)
{
    ...
    testResults.push_back(convertMove(oldPieces, test[j], pieces[newPiece].getTeam(), false));
...
void promotePawn(std::vector<Piece> &pieces, int i, int turnNum, int offerTurn, bool &wasOffered, bool &wasClaimed, bool caughtUp, std::vector<std::string> &oldInputs, std::vector<std::string> &newInputs, int &inputsRead)
{
    ...
    if(wasOffered == false)
        output.append("enter 'offer draw' if you wish to offer a draw, then ");
    output.append("select a piece to promote to by entering \n'q' for queen, 'r' for rook, 'b' for bishop, or 'n' for knight: \n");
    ...
    while(newVal == ' ')
    {
        ...
        else if((newVal == 'o' || newVal == 'c') && wasOffered == false)
        {
            ...
            std::string myOffer;
            int s{0};
            while(s < 2 && std::cin.peek() != 10)
            {
                //char c = std::cin.get();
                myOffer.append(1, std::cin.get());
                if(myOffer[myOffer.size()-1] == ' ')
                    s++;
            ...
            std::cout << "myOffer ==" << myOffer << "..." << std::endl;

            output.append(printBoard(pieces, pieces[i].getTeam(), false, " ", 10));

            if(myOffer == "offer draw")
            {
                wasOffered = true;
                output.append("\nDraw offer extended. \nPlease enter 'q', 'r', 'b', or 'n': ");
            }
            else if(myOffer == "claim draw")
            {
                wasClaimed = true;
                wasOffered = true;
