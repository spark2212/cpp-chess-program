...
int main(int argc, char **argv)
{
    ...
    if(magicWord != "skip")
    {
        std::cout << "In the future, you may skip these instructions by entering 'skip' at any time. Press enter to begin. " << std::flush;
    ...
    while(gameOver == false)
    {
        ...
        //formatSize = fNewInput.size();
        //std::cout << formatSize << " == formatSize for move " << std::flush;
        fNewInput.clear();
        ...
        while(gameOver == false) //Start of inner loop. This loop iterates every attempted move.
        {
            ...
            //next = std::cin.peek(); //this is for draws
            ...
            else if(caughtUp == true)
            {
                ...
                else if((buffer == "i" || buffer == "claim" || buffer == "offer" || buffer == "accept"))
                {
                    ...
                    if(offerTurn == oppoTurn && end == "draw")
                    {
                        ...
                        oldInputs.push_back(end);
                        if(turn == WHITE)
                            finput.append("(=)"); //This is the one
                        else
                        {
                            //finput.erase(finput.size());
                            finput.append("(=)"); // \t
                            //finput.insert(finput.size()-2, )
                        }
            ...
            {          
                ...
                if(resume == false)
                {
                    ...
                    else if(turn == BLACK)
                    {
                        ...
                        if(std::count(finput.end()-6, finput.end(), '\n') > 0)
                            fNewInput.append("\t");
                        //std::cout << "count == " << std::count( finput.end()-8, finput.end(), '\n') << std::flush;
