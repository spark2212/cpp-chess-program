...
//Threads
/*void cinThread(auto T)
{
    std::cin >> T;
}

std::thread th1(cinThread, auto T)*/
...
std::string printTurn(int turnNum, int color, int offerTurn, bool manualClaim, bool wasOffered, bool wasClaimed, int next, bool valid);
//Finders
...
void promotePawn(std::vector<Piece> &pieces, int i, int turnNum, int offerTurn, bool wasOffered, bool wasClaimed, bool display, std::vector<std::string> &oldInputs, int &inputsRead);
...
//Timing functions
inline void pauseRun(int milliseconds, bool display){
...
int main(int argc, char **argv)
{
    ...
    std::cout << "Welcome to ChessGame10.cpp! \n" << std::endl;
    std::string finput{""};
    bool display{true};
    bool resume{false};
    ...
    //std::string devTest;
    //devTest.append(printBoard(startPieces, turn, manualClaim, finput, 10));
    //std::cout << devTest << "\n\ndevTest: this is printBoard with startPieces. " << std::flush;
    ...
    if(argc == 2)
    {
        ...
        while(choice != "auto" && choice != "manual")
        {
            ...
            else
            {
                ...
                std::cout << "\nPlease enter either 'auto' or 'manual': ";
    ...
    std::cin.putback('\n');
    std::cin.getline(std::cin, magicWord, '\b');

    output1.append(printBoard(pieces, turn, manualClaim, finput, 10));
    output1.append(printTurn(turnNum, turn, offerTurn, manualClaim, wasOffered, wasClaimed, 10, true));

    for(int num = 0; num < 3; num++)
        std::cout << "\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n";

    std::cout << "\nWelcome to ChessGame10.cpp! \n\nTo play, enter your desired move or moves in either chess notation or by entering the start and end squares of the desired move.\n" << std::endl;
    
    std::chrono::steady_clock::time_point t = std::chrono::steady_clock::now();

    while(magicWord != "skip" && std::chrono::steady_clock::now() - t <= std::chrono::milliseconds(3000));
    ...
    if(manualClaim == true)
        std::cout << "or claim " << std::flush;

    std::cout << "a draw, attach the appropriate command after your move before entering.\n" << std::endl;

    if(magicWord != "skip")
    {
        pauseRun(15000, true);
        std::cout << "In the future, you may skip these instructions by typing 'start skip' instead of 'start'. Press enter to begin. " << std::flush;
    ...
    //const int gameStart = std::cin.end;
    ...
    while(gameOver == false)
    {
        ...
        while(gameOver == false)
        {
            ...
            if(turnNum == newTurnNum && turn == newTurn)
            {
                caughtUp = true;
                display = true;
                newTurnNum = 0;
                ...
                outputA.append(printTurn(turnNum, turn, offerTurn, manualClaim, wasOffered, wasClaimed, 10, true));
                std::cout << outputA /*<< "All caught up!" */<< std::flush;

                outputA.clear();

                while(oldInputs.size() > inputsRead)
                {
                    oldInputs.pop_back();
                    std::cout << "oldInputs.size() == " << oldInputs.size() << std::flush;
                }

                std::cout << " inputsRead == " << inputsRead << std::flush;
            ...
            {
                std::cout << "turnNum == " << turnNum << "\nnewTurnNum == " << newTurnNum << "\nturn == " << turn << "\nnewTurn == " << newTurn << std::flush;
                std::cout << "\ninputsRead == " << inputsRead << std::endl;
                start = oldInputs[inputsRead];
                inputsRead++;
                end = oldInputs[inputsRead];
                inputsRead++;
                    ...
                    //{
                        ...
                        {  
                            ...
                            if(newTurnNum == 0)
                            {
                                ...
                                output.append(printTurn(turnNum, turn, offerTurn, manualClaim, wasOffered, wasClaimed, next, valid));
                            ...
                            while(newTurnNum > turnNum || newTurnNum <= 0)
                            {
                                std::cin.clear();
                                std::cin.ignore(10000, '\n');
                                std::cout << outputA << "\nPlease enter the turn number and player to move ('white' or 'black') \nyou would like to return to, or enter 'cancel' to cancel: " << std::flush;
                            ...
                            std::cout << "player == " << player << std::flush;
                        ...
                        }while(newTurnNum > turnNum || (newTurnNum == turnNum && newTurn <= turn));
                        ...
                        if(alternate != "cancel")
                        {
                            ...
                            turnNum = 1;
                            outputA.clear();
                            while(turnNum < newTurnNum || (turnNum == newTurnNum && turn > newTurn))
                            {
                                /*fNewInput.clear();
                                ...
                                /*fNewInput.append(convertNotation())
                                start = oldInputs[inputsRead++];
                                end = oldInputs[inputsRead++];
                                i = findPiece(pieces, start);
                                pMoved = pieces[i].getType();
                                movePiece(pieces, start, end, turn);
                            ...
                            /*std::cout << "Moves undone! newTurnNum == " << newTurnNum << "\nand newTurn == " << std::flush;
                            if(newTurn == WHITE)
            ...
            if(valid == false && moveEntered == true)
            {
                ...
                output.append(printTurn(turnNum, turn, offerTurn, manualClaim, wasOffered, wasClaimed, 10, valid));
            ...
            else
            {
                ...
                else if(start == "offer" && end == "draw" && wasOffered == false)
                {
                    ...
                    output.append(printTurn(turnNum, turn, offerTurn, manualClaim, wasOffered, wasClaimed, next, moveEntered));
                    std::cout << output << std::flush;
                    output.clear();
                ...
                else if(start == "claim" && end == "draw" && manualClaim == true && wasClaimed == false)
                {
                    ...
                    else
                    {
                        output.append(printBoard(pieces, turn, manualClaim, finput, 10));
                        output.erase(output.end()-1);
                        output.append(printTurn(turnNum, turn, offerTurn, manualClaim, wasOffered, wasClaimed, 10, moveEntered));
                        std::cout << output << std::flush;
                        output.clear();
                        gameOver = false;
                        continue;
            ...
            {    
                promotePawn(pieces, i, turnNum, offerTurn, wasOffered, wasClaimed, display, oldInputs, inputsRead); //promotes the pawn to the desired piece
            ...
            outputA.append(printTurn(turnNum, oppoTurn, offerTurn, manualClaim, wasOffered, wasClaimed, next, valid)); 
            ...
            if(std::cin.peek() != '\n')
                outputA.append("Executing multiple moves... ");
        ...
        {
            std::cout << outputCopy << "Threefold repetition rule invoked! It's a draw! " << std::endl;
            finput.append("\n\n\t1/2 - 1/2");
            output.clear();
            break;
        }
        ...
        {
            std::cout << outputCopy << "50-move rule invoked! It's a draw! " << std::endl;
            finput.append("\n\n\t1/2 - 1/2");
            output.clear();
            break;
        }
        ...
        if(gameOver == false && display == true)
        {
            std::cout << output << std::flush;

            if(next != 10)
                pauseRun(1500, display);
...
std::vector<std::string> convertMove(std::vector<Piece> &pieces, std::string move, int turn) // search here
{
    std::vector<std::string> route;
...
std::string convertNotation(std::vector<Piece> oldPieces, std::vector<Piece> pieces, pieceType pMoved, std::string move, std::string start, std::string end, int turn)
{
    ...
    testResults.push_back(convertMove(oldPieces, test[j], pieces[newPiece].getTeam()));

    while(testResults[0][1] != end)
    {
        //testResults.push_back();
        //if(testResults[j] == end)
    ...
    input = test[j];
    std::cout << "input for finput is " << input;
    ...
    else
    {
        for(int i = 0; i < MAX_SQUARES; i++)
        {
            if(i == findPiece(pieces, end) || pieces[i].getTeam() != turn || oldPiece[i].getType() != pMoved)
                continue;
            else if(oldPieces[i].getFile() != oldPieces[newPiece].getFile() && oldPieces[i].getRank() != oldPieces[newPiece].getRank())
                continue;
            else if()
        }
    }*/
...
std::string printTurn(int turnNum, int color, int offerTurn, bool manualClaim, bool wasOffered, bool wasClaimed, int next, bool valid) 
{
    ...
    else if(wasOffered == true && (offerTurn != color || checkBack2 != offerTurn))
    {
        ...
        claimClause = ", or enter 'claim draw' to initiate a draw claim";
...
void promotePawn(std::vector<Piece> &pieces, int i, int turnNum, int offerTurn, bool wasOffered, bool wasClaimed, bool display, std::vector<std::string> &oldInputs, int &inputsRead)
{
    ...
    output.append(printTurn(turnNum, -1*pieces[i].getTeam(), offerTurn, false, wasOffered, wasClaimed, 10, true)); //prints the board and pieces
    if(display == true)
        std::cout << output << std::flush;

    output.clear();

    pauseRun(2000, display);
    ...
    if(std::cin.peek() != 10 && display == true)
    {
