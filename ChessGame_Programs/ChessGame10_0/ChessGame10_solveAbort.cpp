...
void promotePawn(std::vector<Piece> &pieces, int i, int turnNum, int offerTurn, bool wasOffered, bool wasClaimed, bool caughtUp, std::vector<std::string> &oldInputs, std::vector<std::string> &newInputs, int &inputsRead);
...
int main(int argc, char **argv)
{
    ...
    std::thread th1(cinThread, std::ref(magicWord), "skip");
    th1.detach();
    ...
    while(magicWord != "skip" && std::chrono::steady_clock::now() - t <= std::chrono::milliseconds(4000));

    std::cout << "If you plan to offer or claim a draw at the end of your turn and you are promoting a pawn, include the promotion when entering your move, e.g. 'exd8=Q offer draw' or 'e7 d8 q offer draw'.\n" << std::endl; 

    t = std::chrono::steady_clock::now();
    while(magicWord != "skip" && std::chrono::steady_clock::now() - t <= std::chrono::milliseconds(5500));
    ...
    t = std::chrono::steady_clock::now();
    while(magicWord != "skip" && std::chrono::steady_clock::now() - t <= std::chrono::milliseconds(15000));

    if(magicWord != "skip")
    {
        std::cout << "In the future, you may skip these instructions by entering 'skip' at any time. Enter 'skip' to begin. " << std::flush;
        //std::cin.clear();
        //std::cin.ignore(10000, '\n');
        
        while(magicWord != "skip");
    ...
    while(gameOver == false)
    {
        ...
        while(gameOver == false) //Start of inner loop. This loop iterates every attempted move.
        {
            ...
            if(caughtUp == false)
            {
                ...
                else if(turnNum == newTurnNum && turn == newTurn/*&& newInputs[inputsRead+1] != "draw"*/)
                {
                    caughtUp = true;
                    //newTurnNum = 0;
                    //newTurn = EMPTY;
            ...
            else if(caughtUp == true)
            {
                ...
                if(buffer == "undo")
                {
                    ...
                    {  
                        ...
                        std::cout << "Return to this position? \nEnter 'yes' or 'no', or enter 'cancel' to cancel: " << std::endl;
                        ...
                        while(confirm[0] != 'y' && confirm[0] != 'n' && confirm != "cancel")
                        {
                            ...
                            std::cout << "Please enter 'yes', 'no', or 'cancel.\nReturn to this position? " << std::endl;
                    ...
                    if(alternate == "cancel")
                    {
                        ...
                        output.clear();
                        output.append(printBoard(pieces, turn, manualClaim, finput, 10));
                        output.append(printTurn(turnNum, turn, offerTurn, manualClaim, wasOffered, wasClaimed, next, true));
            ...
            {          
                ...
                {
                    //turnStr = std::to_string(turnNum);
                    //std::cout << turnStr << std::flush;
                    //if(turn == BLACK && std::count(finput.end()-6, finput.end(), '\n') != 0)
                        //finput.append("\t");
                    ...
                    {
                        //int size = std::count(finput.end()-7, finput.end(), '\n');

                        //int size2 = std::count(finput.end()-4, finput.end(), '\n');
                        //fNewInput.append("\t\t");
                        /*if(turnNum == 1)
                        {
                            size -= 2;
                        ...
                        //if(size2 > 0)
                            //fNewInput.append("\t");
                            //fNewInput.erase(fNewInput.size()-1);
                        //std::cout << "count == " << std::count( finput.end()-8, finput.end(), '\n') << std::flush;
                        fNewInput.append("\t");
            ...
            {    
                promotePawn(pieces, i, turnNum, offerTurn, wasOffered, wasClaimed, caughtUp, oldInputs, newInputs, inputsRead); //promotes the pawn to the desired piece
            ...
            outputA.append(printTurn(turnNum, oppoTurn, offerTurn, manualClaim, wasOffered, wasClaimed, oldNext, valid)); 
            ...
            else if(countMoves > 0 && wasOffered == true && offerTurn == oppoTurn)
            {
                ...
                //if(oppoTurn == BLACK && std::count(finput.end()-7, finput.end(), '\n') != 0)
                    //finput.append("\t");
