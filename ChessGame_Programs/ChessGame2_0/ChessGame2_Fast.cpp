...
//Printers
std::string printBoard(std::vector<Piece> &pieces, int turn, bool manualClaim, std::string finput);
std::string printTurn(int turnNum, int color, bool manualClaim, bool wasOffered);
...
int main(int argc, char **argv)
{
    ...
    int turn{WHITE};
    int oppoTurn{BLACK};
    std::cout << printBoard(pieces, turn, manualClaim, finput);
    static int getDrawNum = getDraw(pieces, turn, PAWN);
    bool wasOffered{false};
    std::cout << printTurn(turnNum, turn, manualClaim, wasOffered);

    while(1)
    {
        static bool newMove{true};
        std::string move;
        ...
        if(newMove == false)
        {
            std::string output = printTurn(turnNum, turn, manualClaim, wasOffered);
            std::cout << output;
        }
        ...
        if(start == "i" && end == "resign")
        {
            if(turn == WHITE)
            {
                ...
                std::string output = printBoard(pieces, oppoTurn, manualClaim, finput);
                std::cout << output;
        ...
        {
            std::string output = printBoard(pieces, turn, manualClaim, finput);
            std::cout << output;
        ...
        else if((start == "offer" || (start == "claim" && manualClaim == true)) && end == "draw")
        {
            ...
            while(response == "")
            {
                ...
                if(response != "yes" && response != "no")
                {
                    if(response == "flip")
                    {
                        std::cout << printBoard(pieces, oppoTurn, manualClaim, finput);
        ...
        {
            /*printBoard(oldPieces, oppoTurn, false, " ");
            ...
            {    
                oldPieces = pieces;
                std::cout << printBoard(pieces, turn, manualClaim, finput); //prints the board and pieces
            ...
            {
                if(pieces[a].getTeam() == EMPTY)
                    continue;
                else if(pieces[a].getTeam() == turn)
                    pieces[a].setEPVul(false);
                else if(pieces[a].getTeam() != turn)
                    pieces[a].setCanEP(false);
            }

            printBoard(oldPieces, turn, false, " ");
            output.append(printTurn(turnNum, oppoTurn, manualClaim, wasOffered));
            ...
            output.append(printBoard(pieces, turn, manualClaim, finput));
            std::cout << output;
        }
        ...
        {
            std::cout << "Fivefold repetition rule invoked! Automatic draw! " << std::endl;
            finput.append("\n\n\t1/2 - 1/2");
            break;
        }
        ...
        output = printTurn(turnNum, turn, manualClaim, wasOffered);
        //output.append(printTurn(turnNum, turn, manualClaim, wasOffered));
        std::cout << output;
    } //End of loop
...
std::string printTurn(int turnNum, int color, bool manualClaim, bool wasOffered) //This function takes an argument BLACK or WHITE and the current turn number and prints a corresponding string
{
    std::string p1 = "White";
    std::string p2 = "Black";
