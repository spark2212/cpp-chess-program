...
void promotePawn(std::vector<Piece> &pieces, int i, int viewTurn, int oppoView, int turnNum, int &offerTurn, bool &wasOffered, bool &wasClaimed, bool caughtUp, std::vector<std::string> &oldInputs, std::vector<std::string> &newInputs, int &inputsRead, std::string finput, bool &needsEntry);
...
int main(int argc, char **argv)
{
    ...
    std::vector<std::vector<std::vector<std::string>>> oldBoardsBranches;
    int branchNum{1};
    ...
    std::cout << "Welcome to ChessGame15.cpp! \n" << std::endl;
    ...
    output1.append(printBoard(pieces, viewTurn, finput, 10));
    ...
    while(gameMode != QUIT) //When this loop ends, so does the program
    {
        ...
        while(gameOver == false)
        {
            ...
            while(gameOver == false) //Start of inner loop. This loop iterates every attempted move.
            {
                ...
                if(caughtUp == false) 
                {
                    if((turnNum == newTurnNum && turn == newTurn && (newGame == true || undo == true)) || inputsRead == newInputs.size())
                    {
                        undo = false;
                ...
                else if(caughtUp == true)
                {
                    ...
                    //timeDiff = std::chrono::steady_clock::now() - turnTime;
                    ...
                    else if((buffer == "adjourn" || buffer == "pause") && argc == 2)
                    {
                        ...
                        std::cout << oldOutput << "Game adjourned. This game can be resumed by relaunching ChessGame15.cpp \nwith the same command-line arguments and entering 'resume'. Goodbye!" << std::endl;

                        break;
                    }
                    else if(buffer == "undo" && (turn != WHITE || turnNum > 1))
                    {
                        ...
                        {  
                            ...
                            while((newTurnNum > turnNum || newTurnNum <= 0) && alternate != "cancel")
                            {
                                std::cin.clear();
                                std::cin.ignore(10000, '\n');
                                std::cout << oldOutput << " Invalid input. \nPlease enter the turn number and player to move ('white' or 'black') \nyou would like to return to, 'b' to begin browsing through the \nprevious board positions, or enter 'c' to cancel: " << std::flush;
                            ...
                            while(player != "white" && player != "black" && alternate != "cancel" && c != 'b')
                            {
                                std::cin.clear();
                                std::cin.ignore(10000, '\n');
                                std::cout << oldOutput << "\nInvalid input. \nPlease enter the player to move ('white' or 'black'), or enter 'c' to cancel: " << std::endl;
                                std::cin >> player;
                                tolower(player);
                                alternate = player;
                            }
                            ...
                            if(turn_i > 0 && turn_i < oldBoards.size())
                            ...
                            std::cout << "'flip' to flip the board, \n't' to toggle automatic board rotation or enter 'c' to cancel: " << std::flush;
                            ...
                            while(confirm[0] != 'y' && confirm[0] != 'n' && confirm != "cancel")
                            {
                                ...
                                if((confirm == "<" && turn_i > 0) || (confirm == ">" && turn_i < oldBoards.size()) || confirm == "flip" || confirm == "t")
                                {
                ...
                else
                {
                    if(start == "i" && end == "resign")
                    {
                        if(turn == WHITE)
                        {
                            ...
                            if(newGame == false && choice != "review")
                                finput2.append("\nWhite resigns! Black wins! \n\t0 - 1");
                    ...
                    {
                        newOutput.append(printBoard(pieces, viewTurn, finput, next));
                        newOutput.append(printTurn(turnNum, turn, offerTurn, manualClaim, wasOffered, wasClaimed, false, false, 10, argc));
                ...
                {    
                    ...
                    //if(resume == true)
                        //std::cout << "move == " << move << " and move[move.size()-1] == " << move[move.size()-1];
                    promotePawn(pieces, i, viewTurn, oppoView, turnNum, offerTurn, wasOffered, wasClaimed, caughtUp, oldInputs, newInputs, inputsRead, finput, readCin);
                ...
                oldOutput.append(printTurn(turnNum, oppoTurn, offerTurn, manualClaim, wasOffered, wasClaimed, valid, validEntry, next, argc));
            ...
            if(changeView == true)
                ...
                break;
            else if(caughtUp == true && choice != "review" && oppoView != viewTurn)
            {
                std::cout << oldOutput << std::flush;
                pauseRun(1000);
            }
        ...
        if(argc == 2 && choice != "resume" && choice != "quit" && gameMode != REVIEW)
        {
        ...
        else if((newGame == false || gameMode == REVIEW) && choice != "quit")
        {
            if(gameMode == REVIEW && branchLvl <= 1)
            {
        ...
        {
            if(choice != "review")
                std::cout << "Game over. Would you like to review the game? \nEnter 'yes' to enter review mode or enter 'quit' to quit: " << std::endl;
            ...
            if(choice == "review")
            {
                ...
                std::string instructions1st = " Enter '>' to advance forward one move, 'w', 'b', or 'n' respectively to change the perspective to White, Black, or normal (alternating), 'flip' to temporarily flip the board, 'skip+ n' to skip forward n moves, '>>' to skip to the end, 'auto' to watch the game unfold automatically, or simply 'jump' to skip directly to any point in the game. To return to the previous position viewed, enter 'last': ";

                formatString(instructions1st, 3, newppgh);
                ...
                formatString(instructions, 3, newppgh);
                ...
                formatString(instructionsLast, 3, newppgh);
                ...
                for(int n = 0; n < oldBoards.size(); n++)
                {
                    ...
                    for(int z = 0; z < 4; z++)
                    {
                        ...
                        std::string autoInstructions = " This game will advance automatically by one ply (half-move) every two seconds. Enter 's' at any time to exit AUTO mode.";
                ...
                //if(save == true)
                //{
                    //oldBoardsBranches.push_back(oldBoardsAll);
                //}
                ...
                while(navigation != "quit")
                {
                    ...
                    case AUTO:
                        ...
                        while(navigation != "s" && n < oldBoards.size()-2 && oldMode == ALT && std::chrono::steady_clock::now() - t <= std::chrono::milliseconds(3000))
                            std::cout << oldBoardsAuto[FLIP][n] << std::flush;
                        break;
                    
                    default:
                        std::cout << allBoardsShort[mode+21][n] << "INVALID INPUT. \n" << turnStrings[n] << instructions << std::flush;
                        pauseRun(3000);
                        valid = false;
                        mode += 20;
                        break;
                    ...
                    else if(mode == AUTO)
                    {
                        go = true;
                        n++;
                        std::cout << "n == " << n << "and the problem is below me" << std::flush;
                    }
                    else
                        while(go == false);
                    ...
                    else if(navigation == "jump")
                    {
                        ...
                        continue;
                    else if(navigation == "branch" && n < oldBoardsBranches[oldBoardsBranches.size()-1][0].size()-2)
                    {
                        newTurnNum = 1 + n/2;
                        if(n % 2 == 0)
                            newTurn = WHITE;
    ...
    {   
        ...
        for(int i = NUM_RANKS; i > 0; i--)
        {
            for(int row = 0; row < ROWS_PER_RANK; row++)
            {
                ...
                if(i == 7 && row == 0) //This prints the first line of the score sheet
                    chessBoard.append(fpartial[0]);
                else if(i == 7 && row == 1)
                    continue;
                else if(i > 1 && i < 8) //This prints out the final twenty moves of the score sheet.
                {
                    //if(y == 1)
                        //y++;
                    
                    while(fpartial.size()-y > partialSize)
                        y++;
...
void promotePawn(std::vector<Piece> &pieces, int i, int viewTurn, int oppoView, int turnNum, int &offerTurn, bool &wasOffered, bool &wasClaimed, bool caughtUp, std::vector<std::string> &oldInputs, std::vector<std::string> &newInputs, int &inputsRead, std::string finput, bool &needsEntry)
{
    ...
    outputTemp.append(printTurn(turnNum, oppoView, offerTurn, false, wasOffered, wasClaimed, true, true, 10, 1)); //prints the board and pieces
