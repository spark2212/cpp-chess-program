...
void promotePawn(std::vector<Piece> &pieces, int i, int viewTurn, int oppoView, int turnNum, int &offerTurn, bool &wasOffered, bool &wasClaimed, bool caughtUp, std::vector<std::string> &oldInputs, std::vector<std::string> &newInputs, int &inputsRead, std::string finput, bool &needsEntry, int* branchPoint, modeType gameMode, int branchLvl);
...
void cinThread2(std::string &T, bool &last, bool &go, bool &skip, int &mode, modeType &gameMode, int branchLvl)
{
    while(1)
    {
        ...
        go = true;
        skip = true;
        if(T == "quit" || (T == "branch" && last == false) || (T == "return" && branchLvl > 0))
            break;
...
/*void coutThread(std::string &newOutput, bool &gameStarted, bool &isTimed, bool &countDown, bool &tickTockW, bool &tickTockB, int &turn, int &viewTurn, bool &gameOver, bool &flag, int &wTime, int &bTime)
{
    while(gameStarted == false)
        std::thread::this_thread::yield();

    std::vector<int> timerPoint; 
    std::vector<std::vector<int>> clocks;

    do{
        timerPoint.clear();
        clocks.clear();
        std::string wClock, bClock;
        std::vector<std::string> timesLeft;

        while(countDown == false && flag == false && gameOver == false)
            std::thread::this_thread::yield();

        if(viewTurn == WHITE)
        {
            timerPoint.push_back(newOutput.find("BLACK"));
            timerPoint.push_back(newOutput.find("WHITE"));
            clocks.push_back(convertToMinutes(bTime));
            clocks.push_back(convertToMinutes(wTime));

            bClock.append(std::to_string(clocks[0][0]));
            bClock.append(":");
            bClock.append(std::to_string(clocks[0][1]));

            wClock.append(std::to_string(clocks[1][0]));
            wClock.append(":");
            wClock.append(std::to_string(clocks[1][1]));

            timesLeft.push_back(bClock);
            timesLeft.push_back(wClock);
        }
        else
        {
            timerPoint.push_back(newOutput.find("WHITE"));
            ...
            wClock.append(std::to_string(clocks[0][0]));
            wClock.append(":");
            wClock.append(std::to_string(clocks[0][1]));

            bClock.append(std::to_string(clocks[1][0]));
            bClock.append(":");
            bClock.append(std::to_string(clocks[1][1]));

            timesLeft.push_back(wClock);
            timesLeft.push_back(bClock);
        }
        
        std::cout << "\f" << newOutput.substr(0, timerPoint[0]+5);
        if(isTimed == true)
            std::cout << " " << timesLeft[0];
        
        std::cout << newOutput.substr(timerPoint[0]+6, timerPoint[1]+5);
        if(isTimed == true)
...
/*void cinMoveThread(std::string &T, bool &flag)
{
    bool done{false};
    std::thread threadThread(basicCinThread, std::ref(T), std::ref(done));
...
int main(int argc, char **argv)
{
    ...
    std::cout << "Welcome to ChessGame16.cpp! \n" << std::endl;
    ...
    else if(argc == 3)
    {
        ...
        while(1)
        {
            ...
            while(choice != "copy" && choice != "move" && !(choice == "swap" && file2 && file2Size != 0) && choice != "quit")
            {
                ...
                std::cout << "\nOtherwise, enter 'quit': " << std::flush;
    ...
    while(choice != "quit" && choice != "start" && ((choice != "resume" && choice != "review") || argc != 2))
    {
        ...
        else
        {
            ...
            std::cout << "\nPlease enter ";
            if(argc == 2)
                std::cout << "'start', 'quit', 'resume' or 'review': " << std::flush;
            else
                std::cout << "'quit' or 'start': " << std::flush;
    ...
    bool tickTockW{false};
    bool tickTockB{false};
    bool playWithClock{false};
    bool gameStarted{false};*/
    bool flag{false};

    //std::thread timeThread(coutThread, std::ref(newOutput), std::ref(gameStarted), std::ref(countDown), std::ref(tickTockW), std::ref(tickTockB), std::ref(turn), std::ref(gameOver), std::ref(flag));
    //timeThread.detach();
    ...
    if((choice == "resume" || choice == "review") && argc == 2)
    {
        ...
        while(1)
        {
            ...
            else if(extraHead[0] != ' ' && extraHead.find("vs.", 0) != std::string::npos && extraHead.find("vs.", 0) != extraHead.size()-3 &&extraHead[extraHead.size()-1] >= '!' && reformat == true)
            {
                extraHead.insert(extraHead.begin(), ' ');
                extraHead.insert(extraHead.end()-1, ' ');
                i += 2;
                reformat = false;
            }
            ...
            if(i >= 4)
            {
                ...
                {
                    extraHead.insert(extraHead.end()-4, ' ');
                    oldLog.unget();
                }
            }
            ...
            if(i < 5)
                i++;
            else if(extraHead[i-5] == '\n' && (extraHead[i-4] == ' ' || extraHead[i-4] == '*') && extraHead[i-3] == '1' && extraHead[i-2] == '.' && extraHead[i-1] == ' ')
            {
                oldLog.unget();
                break;
            }
        ...
        while(oldLog.good() == true)
        {
            ...
            while(oldLog.good() == true)
            {
                ...
                if(i < 1)
                    ...
                    i++;
                else if(extraHead[i] == ' ' && extraHead[i-1] != '.')
                    i++;
                else
                {
                    gotOffer = false;
    ...
    {
        bool undo{false};
        while(gameOver == false && gameMode != RETURN)
        {
            std::string move;
            std::string buffer;
            ...
            while(gameOver == false) //Start of inner loop. This loop iterates every attempted move.
            {
                ...
                if(caughtUp == false) 
                {
                    ...
                    if((turnNum == newTurnNum && turn == newTurn && (newGame == true || undo == true || gameMode == BRANCH)) || (inputsRead == newInputs.size()))
                    {
                        undo = false;
                        if(gameMode != BRANCH && gameMode != RETURN)
                        {
                    ...
                    if(newInputs[inputsRead] == "star")
                    {
                        if(finput[finput.size()-1] != '*')
                            finput.append("*");
                        oldInputs.push_back("star");
                        inputsRead++;
                        continue;
                    }
                    ...
                    else */if(newInputs[inputsRead] == "resign")
                    {
                        start = "i";
                        end = "resign";
                        inputsRead++;
                    ...
                    else if(resume == true)*/
                    {
                        ...
                        if(move == "offer")
                        {
                            //std::cout << "move == offer on move " << turnNum << std::endl;
                            //start = "offer";
                            //end = "draw";
                ...
                else if(caughtUp == true)
                {
                    ...
                    if(buffer == "return" && branchLvl > 0)
                    {
                        ...
                        /*std::string amount;
                        bool done{false};
                        std::thread returnThread(basicCinThread, std::ref(amount), std::ref(done));
                        if(done == false)
                        {
                            std::cin.unget();
                            amount = "one";
                        ...
                        while(oldMovesCatalog.size() > branchLvl+1)
                            oldMovesCatalog.pop_back();

                        //std::cout << "oldMovesCatalog.size() == " << oldMovesCatalog.size() << "\nand oldMovesCatalog[branchLvl].size() == " << oldMovesCatalog[branchLvl].size() << std::endl;
                        //while(std::cin.get() != '\n');

                        //while(n_remembered.size() > branchLvl+1)
                        ...
                        wasClaimed = false;
                        offerTurn = WHITE;
                        newTurnNum = oldEndings[branchLvl][0];
                        newTurn = oldEndings[branchLvl][1];
                        turn = WHITE;
                        oppoTurn = BLACK;
                        turnNum = 1;
                    ...
                    else if(buffer == "adjourn" && gameMode == PLAY && argc == 2) 
                    {
                        ...
                        std::cout << oldOutput << "Game adjourned. This game can be resumed by relaunching ChessGame16.cpp \nwith the same command-line arguments and entering 'resume'. Goodbye!" << std::endl;
                ...
                {          
                    ...
                    if(gameMode == REVIEW && (turnNum > branchPoint[0] || (turnNum == branchPoint[0] && turn < branchPoint[1])))
                    {
                        static int width = finput2.size();
                        if(wasOffered == true && offerTurn == turn)                         
                            finput2.append("(=)");
                        
                        if(finput2.size()-width >= 20)
                        {
                            finput2.append("\n");
                            width = finput2.size();
                        }
                        ...
                        finput2.append(turnStr);
                        finput2.append(2-turn, '.');
                        finput2.append(" ");
                        finput2.append(newInput);
                    }   
                ...
                {    
                    ...
                    promotePawn(pieces, i, viewTurn, oppoView, turnNum, offerTurn, wasOffered, wasClaimed, caughtUp, oldInputs, newInputs, inputsRead, finput, readCin, branchPoint, gameMode, branchLvl);
                    ...
                    if(gameMode == BRANCH && std::count(newInput.begin(), newInput.end(), '=') == 0)
                    {
                        finput2.append("=");
        ...
        {
            while(oldEndings.size() > branchLvl+1)
                oldEndings.pop_back();
        }
        //if(decision == true)
        //bool save{true};
        ...
        {
            if(gameMode == BRANCH && branchLvl == 1)
            {
                std::cout << outputCopy << "\nWould you like to save this alternate ending? Enter 'y' or 'n': " << std::endl;
                ...
                {
                    std::cout << outputCopy << "\nInvalid input. Enter 'y' or 'n': " << std::endl;
                    std::cin >> choice2;
                    tolower(choice2);
                }
        ...
        if(decision == true && choice != "quit")
        {
            ...
            if(choice == "review")
            {
                ...
                //if(gameMode != RETURN)
                {
                    ...
                    std::string instructions = " Enter '>' to advance forward one move, '<' to return to the previous move, 'w', 'b', or 'n' respectively to change the perspective to White, Black, or normal (alternating), 'flip' to temporarily flip the board, 'skip+ n' or 'skip- n' to skip forward or back n moves, '<<' to skip to the beginning, '>>' to skip to the end, 'auto' to watch the game unfold automatically, or simply 'jump' to skip directly to any point in the game. To return to the previous position viewed, enter 'last': ";

                    std::string instructionsLast = " Enter '<' to return to the previous move, 'w', 'b', or 'n' respectively to change the perspective to White, Black, or normal (alternating), 'flip' to temporarily flip the board, 'skip- n' to skip back n moves, '<<' to skip to the beginning, or simply 'jump' to skip directly to any point in the game. To return to the previous position viewed, enter 'last': ";
                ...
                while(navigation != "quit")
                {
                    ...
                    case AUTO:
                        ...
                        {
                            if(once++ == 0) 
                                std::cout << oldBoardsAuto[FLIP+1][n] << std::flush;
                    ...
                    else if((navigation == "skip+" && n < oldBoards.size()-1) || (navigation == "skip-" && n > 0))
                    {
                    ...
                    else if(navigation == "jump")
                    {
                        ...
                        continue;
                    else if(navigation == "branch" && n < oldBoards.size()-2)
                    {
                        gameMode = BRANCH;
                        navigation = "\n";     
                        newTurnNum = 1 + n/2;
                        ...
                        //std::cout << "branchPoint[0] == newTurnNum == " << branchPoint[0] << " \nand branchPoint[1] == newTurn == " << branchPoint[1] << std::endl;
                        
                        viewMode = mode;
                        oldViewMode = oldMode;

                        //std::cout << "oldInputs[0] == " << oldInputs[0] << " and oldInputs[1] == " << oldInputs[1] << std::endl;
                    ...
                    else if(navigation == "return" && branchLvl > 0)
                    {
                        ...
                        std::cout << "oldMovesCatalog.size() == " << oldMovesCatalog.size() << std::endl;
                        while(std::cin.get() != '\n');
                        newInputs = oldMovesCatalog[oldMovesCatalog.size()-1];
                        //n = n_remembered[n_remembered.size()-1];
                        for(int i = 0; i < newInputs.size(); i++)
                        {
                            std::cout << newInputs[i] << std::endl;
                        }
                        while(std::cin.get() != '\n')
                    ...
                    else if(navigation == "last" && last_n >= 0)
                    {
                        int temp_n = n;
                        n = last_n;
                        last_n = temp_n;
