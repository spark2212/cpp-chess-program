        ...
        if(decision == true && choice != "quit")
        {
            ...
            if(choice == "review")
            {
                ...
                if(gameMode != RETURN)
                {
                    ...
                    instructionsLast = " Enter '<' to return to the previous move, 'w', 'b', or 'n' respectively to change the perspective to White, Black, or normal (alternating), 'flip' to temporarily flip the board, 'skip- n' to skip back n moves, '<<' to skip to the beginning, or simply 'jump' to skip directly to any point in the game. To return to the previous position viewed, enter 'last': ";
                    ...
                    if(branchLvl > 0)
                    {
                        ...
                        for(int i = 1; i < oldBoards.size()-2; i++)
                ...
                while(navigation != "quit")
                {
                    ...
                    default:
                        ...
                        //if(n < oldBoards.size()-1)
                            std::cout << instructionsManual[n] << std::flush;
                        //else
                            //std::cout << instructionsLast << std::flush;
                        pauseRun(1000);
                        valid = false;
                        mode += 20;
                        break;
