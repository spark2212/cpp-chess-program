...
void cinThread2(std::string &T, bool &last, bool &go, bool &skip, int &mode, modeType &gameMode, int branchLvl)
{
    while(1)
    {
        while(skip == true || gameMode == RETURN || mode < -1)
        ...
        while(gameOver == false && gameMode != RETURN)
        {
            ...
            while(gameOver == false) //Start of inner loop. This loop iterates every attempted move.
            {
                ...
                else if(caughtUp == true)
                {
                    ...
                    else if((buffer == "adjourn" || buffer == "pause") && argc == 2)
                    {
                        gameOver = true;
                        decision = false;
        ...
        if(decision == true && choice != "quit")
        {
            ...
            if(choice == "review")
            {
                ...
                if(gameMode != RETURN)
                {
                    ...
                    std::cout << "instructionsManual.size() == " << instructionsManual.size() << "\nand oldBoards.size() == " << oldBoards.size() << std::endl;

                    //std::cout << "instructionsLast == " << instructionsLast << std::endl;
                    //std::cout << instructionsManual[oldBoards.size()-2] << std::endl;
                }
                ...
                while(navigation != "quit")
                {
                    ...
                    {
                        gameMode = RETURN;
                        std::string part2{"one"};
                        std::thread returnThread(cinThread, std::ref(part2), "all");
                        returnThread.detach();

                        if(part2 == "all")
                        {
                            while(n_remembered.size() > 1)
                                n_remembered.pop_back();

                            while(oldMovesCatalog.size() > 1)
