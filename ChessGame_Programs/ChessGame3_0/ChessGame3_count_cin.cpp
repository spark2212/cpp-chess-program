...
std::string printBoard(std::vector<Piece> &pieces, int turn, bool manualClaim, std::string finput, int clarity);
void printTurn(int turnNum, int color, bool manualClaim, bool wasOffered);
std::string printTurn(int turnNum, int color, bool manualClaim, bool wasOffered, int clarity);
...
int main(int argc, char **argv)
{
    ...
    int turn{WHITE};
    int oppoTurn{BLACK};
    output1.append(printBoard(pieces, turn, manualClaim, finput, 1));
    static int getDrawNum = getDraw(pieces, turn, PAWN);
    bool wasOffered{false};
    output1.append(printTurn(turnNum, turn, manualClaim, wasOffered, 1));
    ...
    while(1)
    {
        ...
        {
            route = convertMove(pieces, buffer, turn);
            start = route[0];
            end = route[1];
        }
        else
        {//resume here
            ...
            while(1)
            {
                ...
                move = buffer.substr(buffer.begin() + i, sp);
                if(i == 0)
                {
                    for(int j = 0; j < move.size(); j++)
                        start.append(tolower(move[j]));
                }
                else
                {
                    for(int j = 0; j < move.size(); j++)
                        end.append(tolower(move[j]));

                    break;
                }
                
                if(start == "i" || start == "offer" || start == "claim")
                {
                    i += sp;
                }
                else if(findPiece(pieces, start) == -1)
                {
                    start.clear();
                    route = convertMove(pieces, move)
                }
                    if(pieces[findPiece(pieces, start)].getTeam() == ){
                }
            }
            /*int j{0};
            for(int i = 0; i < buffer.size(); i++)
            {
                //if(std::count(move.begin(), move.end(), ' ') )
                if(buffer[i] == ' ' && j == 0)
                {
                    move = buffer.substr(buffer.begin(), i)
                }
            }*/
        }
        ...
        output.append(printBoard(pieces, oppoTurn, false, " ", 1));
        output.append(printTurn(tNum, oppoTurn, manualClaim, wasOffered, 1));
        std::cout << output;

        //std::cout << output << "line 281";
        ...
        {   
            //output.append(word);
            int num1 = findPiece(pieces, move);
            int num2 = findEmpty(pieces, move);
            int num3 = findPiece(pieces, word);

            if(num3 != -1)
            {
                if(pieces[num3].getTeam() == turn)
                {
                    start = word;
                    std::cin >> end;
                }
            }
            else if(num1 == -1 && num2 != -1) //move is an empty square on the board
            {
                route = convertMove(pieces, move, turn);
                start = route[0];
                end = route[1];
            }
            else if(num1 == num2) //move is not on the board
            {
                //This will always be an invalid move
                start = "a1";
                end = "a1";
            }
            else if(pieces[num1].getTeam() != turn) //move is a square occupied by an opponent's piece
            {
                route = convertMove(pieces, move, turn);
                start = route[0];
                end = route[1];
            }
            else //move represents the starting square
            {
                start = move;
                std::cin >> end;
            }
        ...
        {
            if(word == "i" || word == "offer" || word == "claim")
            {
                start = word;
                std::cin >> end;
            }
            else if(move.size() < 2 || move.size() > 6)
            {
                start = "a1";
                end = "a1";
            }
        ...
        std::cout << move;
        //std::thread::detach;*/ //original ends here

        for(int n = 0; n < start.size(); n++)
            start[n] = tolower(start[n]);
        ...
        {
            std::string output;
            output.append(printBoard(pieces, turn, manualClaim, finput, 1));
            output.append("You have already extended a draw offer on this turn.");
            output.append(printTurn(turnNum, turn, manualClaim, wasOffered, 1));
            std::cout << output;
            output.clear();
            continue;
        }
        else if((start == "offer" || (start == "claim" && manualClaim == true)) && end == "draw")
        {
            ...
            while(response == "")
            {
                ...
                if(response != "yes" && response != "no")
                {
                    if(response == "flip")
                    {
                        output.append(printBoard(pieces, oppoTurn, manualClaim, finput, 1));
        ...
        {    
            //std::string output;
            //output.append(printBoard(oldPieces, turn, manualClaim, finput, 1)); //prints the board and pieces
            //output.append(printTurn(turnNum, oppoTurn, manualClaim, wasOffered, 1)); 
            //std::cout << std::endl << output << "line 556";
            //std::chrono::steady_clock::time_point t1 = std::chrono::steady_clock::now();
            //while(std::chrono::steady_clock::now() - t1 <= std::chrono::milliseconds(1000));
            //std::this_thread::sleep_for(std::chrono::milliseconds(1000));
        ...
        /*output.append(printBoard(oldPieces, turn, false, " ", 1));
        output.append(printTurn(turnNum, oppoTurn, manualClaim, wasOffered, 1));
...
std::vector<std::string> convertMove(std::vector<Piece> &pieces, std::string move, int turn)
{
    ...
    //std::cout << "Entered move is " << move << std::endl;
...
std::string printBoard(std::vector<Piece> &pieces, int turn, bool manualClaim, std::string finput, int delta)
{
    std::string chessBoard = "\n\n\n";
...
//Finders/////////////////////////
//This function returns the index of the Piece object that exists at a location, regardless of its team.
int findSquare(std::vector<Piece> pieces, std::string location)
{
...
{
    std::string output;
    output.append(printBoard(pieces, -1*pieces[i].getTeam(), false, " ", 1)); //prints the board and pieces
    ...
    while(newVal == ' ')
    {
        ...
        {
            output.append(printBoard(pieces, -1*pieces[i].getTeam(), false, " ", 1));
